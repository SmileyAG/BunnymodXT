...
struct HwDLL::Cmd_BXT_Skybox_Name
{
	USAGE("Usage: bxt_skybox_name <name>\n");
...
void HwDLL::RegisterCVarsAndCommandsIfNeeded()
{
	...
	wrapper::Add<
		...
		Handler<>,
		Handler<int>>("w");
	wrapper::Add<Cmd_BXT_Skybox_Name, Handler<const char*>>("bxt_skybox_name");
...
HOOK_DEF_0(HwDLL, void, __cdecl, R_SetFrustum)
{
	...
	if (CVars::bxt_fix_widescreen_fov.GetBool())
	{
		...
		if (fov != prev_calc_fov)
		{
			float calc_fov = static_cast<float>(std::atan(std::tan(fov * M_PI / 360.0f) * def_aspect_ratio * our_aspect_ratio) * 360.0f / M_PI);
